...
function createPdf(order:ReportPayload,logoBytes:Uint8Array){
  try{
    ...
    order.panels.forEach((panel,index)=>{panelRows.push([panel.totalPanels,`${index+1}. ${panel.name} - ${panel.coverage}`,panel.gauge,normalizeProductTerminology(panel.finish),panel.color,[panel.pan,panel.notching,panel.roofPitch?`Pitch ${panel.roofPitch}`:"",panel.clip,panel.coil?`Coil ${panel.coil}`:""].filter(Boolean).join(" | ")]);panel.lengths.forEach(row=>panelRows.push([row.qty,`Length: ${row.feet} ft ${row.inches} in`,"","","",""]))});
    ...
    band("ACCESSORIES");
    table(["QTY","UNIT","ITEM DESCRIPTION","UNIT PRICE","LINE TOTAL"],[42,54,270,85,105],order.accessories.length?order.accessories.map(item=>[item.quantity,item.unit,`${item.name} | ${item.part}`,money(item.unitPrice),money(item.lineTotal)]):[[0,"","No accessories selected","",""]]);
    y+=10;band("FLASHINGS");
    table(["QTY","ITEM DESCRIPTION","GA./MATERIAL","COLOR"],[45,286,105,120],order.flashings.length?order.flashings.map(item=>[item.quantity,`${item.name}${item.pitch?` | Pitch ${item.pitch}`:""}`,order.flashingFinish.gauge,order.flashingFinish.color]):[[0,"No flashings selected",order.flashingFinish.gauge,order.flashingFinish.color]]);
